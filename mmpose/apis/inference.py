...
# 신경 안 써도 됨
    ...
    bboxes_ori = np.array(person_results)
    print('bboxes_ori' , bboxes_ori)
    print(len(bboxes_ori))
    # print(len(bboxes))
    # print(bboxes[0][4])
    ...
    for i in range(len(bboxes_ori)):
        if bboxes_ori[i][4] == 0:
            bboxes.append(bboxes_ori[i][0:4])
            # print('cls =', bboxes_ori[i][4])
        else:
            pass
    ...
    for i in range(len(bboxes)):    
        bboxes_xyxy.append(bboxes[i][0:4])
        print(bboxes_xyxy)
        # xywh = bbox_xyxy2xywh(xyxy)
        # bboxes_xyxy.append(xyxy)
        for k in range(len(bboxes_xyxy)):
            x = (bboxes_xyxy[k][0] + bboxes_xyxy[k][2])/2
            y = (bboxes_xyxy[k][1] + bboxes_xyxy[k][3])/2
            width = bboxes_xyxy[k][2] - bboxes_xyxy[k][0]
            height = bboxes_xyxy[k][3] - bboxes_xyxy[k][1]
        # bboxes_xywh.append(bbox_xyxy[2] - bbox_xyxy[0], bbox_xyxy[3] - bbox_xyxy[1], (bbox_xyxy[0] + bbox_xyxy[2])/2, (bbox_xyxy[1] - bbox_xyxy[3])/2)
        # xywh = bbox_xyxy2xywh(xyxy)
            bboxes_xywh = [x, y, width, height]
        # bboxes_xywh.append(xywh)
    # else:
    #     # format is already 'xywh'
    #     bboxes_xywh = bboxes[:, 3]
    ...
    for pose, person_result, bbox_xyxy in zip(poses, person_results,
        ...
        # pose_result.append(np.array(pose))
        pose_result['person_result'] = bboxes
